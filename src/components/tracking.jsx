...
export default function HandTrackingScene() {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const printBoneTree = (bone, indent = "") => {
          console.log(indent + bone.name);
          bone.children.forEach((c) => printBoneTree(c, indent + "  "));
        };
        model.traverse((child) => {
          if (child.isBone && child.parent && !child.parent.isBone) {
            printBoneTree(child); // print whole skeleton
          }
        });
  ...
  // Spine chain
  spineLower: find("DEF-spine"),
  spineMid: find("DEF-spine001"),
  spineUpper: find("DEF-spine002"),
  chest: find("DEF-spine003"),  // chest-level rotation
  upperChest: find("DEF-spine004"),
